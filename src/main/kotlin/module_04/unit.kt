...
/* TODO
    ...
    2. До стрелки -> могут находится параметры (message: String), которые передаются в функцию
 ...
 */
    ...
    print( "ЗАДАЧА 3.\nСделать функцию calculateSavingsAmount из functions.kt\n" +
            "как функцию высшего порядка, т.е. она может принимать в качесте аргумента другую функцию \n" +
            "или лямбда выражения.\n" )
/* TODO
    ЕСЛИ ЛЯМБДА ВЫРАЖЕНИЕ - ЭТО ПОСЛЕДНИЙ АРГУМЕНТ ФУНКЦИИ (в нашем примере это preintInfo
    то по соглашению CODE STYLE он выносится за скобку и выражение:
            "calculateSavingsAmount(30000, 0.12, 8, printInfo = { _, _, _, bank -> println(...)}
    "превращается в:
            "calculateSavingsAmount(30000, 0.12, 8) { _, _, _, bank -> println(...)}
    )
*/
    print ("\nЕСЛИ ЛЯМБДА ВЫРАЖЕНИЕ - ЭТО ПОСЛЕДНИЙ АРГУМЕНТ ФУНКЦИИ (в нашем примере это preintInfo, \n" +
           "то по соглашению CODE STYLE он выносится за скобку и выражение:\n + " +
